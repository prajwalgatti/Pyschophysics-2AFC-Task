...
_delta      	 = transpose(delta_);
_change     	 = transpose(change_);
_alpha      	 = transpose(alpha);
_beta       	 = transpose(beta);
_trialnum		 = trialnum.slice();
_probe_arr  	 = probe_arr.slice();
_reward_cue 	 = reward_cue.slice();
_reward_skew_cue = reward_skew_cue.slice();
_t_cuestim  	 = t_cuestim.slice();

var trial_info_table = {
	Trial_num: 		_trialnum.slice(),
	Probe: 			_probe_arr.slice(),
	Reward_L: 		_reward_cue.slice(),
	RewardSkew_Cue: _reward_skew_cue.slice(),
	Change_Angle_L: _delta[0].slice(),
	Change_Angle_R: _delta[1].slice(),
	Change_L: 		_change[0].slice(),
	Change_R: 		_change[1].slice(),
	TimeCueStim: 	_t_cuestim.slice() 
}
...
var angle_info_table = {
	...
	Probe: 	   _probe_arr.slice(),
	Reward_L:  _reward_cue.slice(),
	Alpha_L:   _alpha[0].slice(),
	Alpha_R:   _alpha[1].slice(),
	Delta_L:   _delta[0].slice(),
	Delta_R:   _delta[1].slice(),
	Beta_L:    _beta[0].slice(),
	Beta_R:    _beta[1].slice()
}
...
while(shuffle_flag == true){
	...
	for(i=0; i<shuffle_seq.length; i++){
		trial_info_table.Probe[i]			= _probe_arr[shuffle_seq[i]];
		trial_info_table.Reward_L[i]		= _reward_cue[shuffle_seq[i]];
		trial_info_table.RewardSkew_Cue[i]	= _reward_skew_cue[shuffle_seq[i]];
		trial_info_table.Change_Angle_L[i]	= _delta[0][shuffle_seq[i]];
		trial_info_table.Change_Angle_R[i]	= _delta[1][shuffle_seq[i]];
		trial_info_table.Change_L[i]		= _change[0][shuffle_seq[i]];
		trial_info_table.Change_R[i]		= _change[1][shuffle_seq[i]];
		trial_info_table.TimeCueStim[i]		= _t_cuestim[shuffle_seq[i]];
		
		angle_info_table.Probe[i]			= _probe_arr[shuffle_seq[i]];
		angle_info_table.Reward_L[i]		= _reward_cue[shuffle_seq[i]];
		angle_info_table.Alpha_L[i]			= _alpha[0][shuffle_seq[i]];
		angle_info_table.Alpha_R[i]			= _alpha[1][shuffle_seq[i]];
		angle_info_table.Delta_L[i]			= _delta[0][shuffle_seq[i]];
		angle_info_table.Delta_R[i]			= _delta[1][shuffle_seq[i]];
		angle_info_table.Beta_L[i]			= _beta[0][shuffle_seq[i]];
		angle_info_table.Beta_R[i]			= _beta[1][shuffle_seq[i]];
	}

	for(i=0; i<numtrials; i++){
		_probe_arr[i] = trial_info_table.Probe[i];
		_reward_cue[i] = trial_info_table.Reward_L[i];
		_reward_skew_cue[i] = trial_info_table.RewardSkew_Cue[i];
		_delta[0][i] = trial_info_table.Change_Angle_L[i];
		_delta[1][i] = trial_info_table.Change_Angle_R[i];
		_change[0][i] = trial_info_table.Change_L[i];
		_change[1][i] = trial_info_table.Change_R[i];
		_t_cuestim[i] = trial_info_table.TimeCueStim[i];
		_alpha[0][i] = angle_info_table.Alpha_L[i];
		_alpha[1][i] = angle_info_table.Alpha_R[i];
		_beta[0][i] = angle_info_table.Beta_L[i];
		_beta[1][i] = angle_info_table.Beta_R[i];
	}

	// _probe_arr 		 = trial_info_table.Probe.slice();
	// _reward_cue 	 = trial_info_table.Reward_L.slice();
	// _reward_skew_cue = trial_info_table.RewardSkew_Cue.slice();
	// _delta 			 = [trial_info_table.Change_Angle_L.slice(), trial_info_table.Change_Angle_R.slice()];
	// _change 		 = [trial_info_table.Change_L.slice(), trial_info_table.Change_R.slice()];
	// _t_cuestim 		 = trial_info_table.TimeCueStim.slice();
	// _alpha 			 = [angle_info_table.Alpha_L.slice(), angle_info_table.Alpha_R.slice()];
	// _beta 			 = [angle_info_table.Beta_L.slice(), angle_info_table.Beta_R.slice()];
